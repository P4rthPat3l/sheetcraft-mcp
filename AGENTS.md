...
- Nothing — all six toolsets have live ops (core 15, drive 7, formatting 5, charts 3, pivot 2, power 2 = 34 ops).
...
- Token budgets (measured via tools/list, chars/3.5): core ≈ 4.6K, all 34 ≈ 10K. Keep descriptions tight when adding ops.
...
- **Credential boundary**: agents may attempt to read `~/.config/sheetcraft-mcp/*` and hand-roll API calls when they believe a tool is missing (observed 2026-08-29: agent extracted the refresh token from the token store rather than retry the MCP after a mid-session update). Mitigations: the bundled skill's Boundaries section forbids it; docs instruct re-checking tool schemas. Keep descriptions accurate so agents don't reach for the filesystem.
- **Array params coerce JSON strings** (`arrayParam` in src/ops/writing-values.ts): smaller models occasionally serialize `writes`/`values`/`rows`/`ranges` as a JSON string → opaque "Expected array, received string" from the SDK. Parse it; teach on wrong shape. Apply to any new array-typed op param.
...
- **Google Tables (typed columns) reject setDataValidation (verified 2026-08-29)**: "This operation is not allowed on cells in typed columns" — applies to API and UI Data-validation dialog. Same op on a non-table column of the same sheet succeeds. Mitigate: detect via sheets.get (table object) or surface the typed-columns hint when this 400 appears; workaround is UI chip-menu or converting the table.
- **Pivot-table writes are silently stripped by the Sheets API (verified 2026-08-28, re-verified 2026-08-29)**: `updateCells` with a `pivotTable` cell value returns success (`replies: [{}]`) but the cell stays empty afterward — tried masks `userEnteredValue`, `*`, and `userEnteredValue.pivotTable` (the last → "Invalid field: userEnteredValue.pivot_table"). Plain writes to the same cell persist, so it's pivot-specific. Same-sheet and cross-sheet pivots both affected. Google's own guide documents this exact request shape; treat API pivot creation as unreliable — create_pivot verifies persistence and warns when the write didn't take.
